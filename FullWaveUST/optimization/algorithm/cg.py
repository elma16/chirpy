"""
FullWaveUST.optimization.algorithm.cg
====================================

Non-linear Conjugate Gradient (CG) optimizer for Helmholtz full-waveform inversion (FWI).
...
API Summary
-----------
CG(mode='real').solve(fun, m0, n_iter=8, viz=viewer)

get_record() → returns a dictionary with 4 arrays (Ny, Nx, N_iter)

reset() — clears all CG state and recorder buffers
...
class CG(Optimizer):
    ...
    def step(self, g_raw: np.ndarray, m: ImageData, fun: NonlinearLS) -> np.ndarray:
        """Single CG iteration; returns raw gradient for next step."""
    ...
    def reset(self):
        """Clear CG state and recorder (e.g. when switching frequency)."""
        self._d = self._g_prev = None
