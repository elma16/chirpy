...
img = ImageData(
    array=model_true, grid=img_grid, tx_array=ring
)  # ImageData for visualization
img.show()
...
array_pair = TransducerArray2D(
    positions=pair_positions, is_tx=is_tx_pair, is_rx=is_rx_pair
)

# B) one TX (top), all RX
is_tx_1 = np.zeros(ring.n_elements, dtype=bool)
is_tx_1[top_idx] = True
is_rx_all = np.ones(ring.n_elements, dtype=bool)
array_1tx_allrx = TransducerArray2D(
    positions=ring.positions, is_tx=is_tx_1, is_rx=is_rx_all
)
...
vals = np.concatenate(
    [np.abs(G_pair).ravel(), np.abs(G_1tx_allrx).ravel(), np.abs(G_full).ravel()]
)
...
)  # no overlap with constrained layout
...
im1 = axes[1].imshow(
    G_1tx_allrx, origin="lower", extent=extent, cmap="seismic", norm=normG
)
